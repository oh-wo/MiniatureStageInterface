...
namespace WpfApplication1
{
    ...
    public class Dxf
    {
        ...
        public void InterpretFile()
        {
            ...
            {
                circle.laserLines = ConvertCircleToLines(circle, MainWindow.lineSpacing);
            }
                ...
                if (pline.noVerticies > 0)
                {
                    for (int i = 0; i < pline.noVerticies; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //link to next vertex in list
                                pline.laserLines.AddRange(ConvertBulgeToLines(pline.verticies[i].Point, pline.verticies[i + 1].Point, pline.verticies[i].Bulge ?? (float)0, MainWindow.lineSpacing, z));
                            }
                            else
                            {
                                if (pline.closed)
                                {
                                    //link back to the original vertex
                                    pline.laserLines.AddRange(ConvertBulgeToLines(pline.verticies[i].Point, pline.verticies[0].Point, pline.verticies[i].Bulge ?? (float)0, MainWindow.lineSpacing, z));
                                }
                            };
                        };
        ...
        private static Dxf dXf = new Dxf();
        ...
        public DxfEventHandler dxfEvent;
        ...
        public static void DxfEventMethod(Dxf dxf, DxfEventArgs e)
        {
