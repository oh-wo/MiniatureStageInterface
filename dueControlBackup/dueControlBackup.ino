...
void MoveAndShoot(float posX, float posY, float positionTolerance) {
  ...
  PulseLaser();
  delay(50);//ensure laser has shott
}
  ...
  while (!posReadComplete) {
    if (Serial1.available()) {
      ...
      if (allDataRead) {
        ...
        if (indexAxisTwoPlusSix>posReadString.length() && debugging) {
          Serial.print("all data read:"); Serial.println(allDataRead);
          Serial.print("indexAxisTwoPlusSix:"); Serial.println(indexAxisTwoPlusSix);
          Serial.print("posReadString.length():"); Serial.println(posReadString.length());
          Serial.print("(indexAxisTwoPlusSix + 7):"); Serial.println((indexAxisTwoPlusSix + 7));
          Serial.print(allDataRead);
        }
        posReadComplete = true;
        actualPosX = StrToFloat(posReadString.substring(indexAxisOnePlusSix, indexAxisOnePlusSix + 7));
        actualPosY = StrToFloat(posReadString.substring(indexAxisOnePlusSix, indexAxisOnePlusSix + 7));
        if (debugging) {
